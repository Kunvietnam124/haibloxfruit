...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>haibloxfruits chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "haibloxfruits", HidePremium = false, IntroText = "haibloxfruits⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("haibloxfruits SCRIPT")
W:AddLabel("Discordhaibloxfruits")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "haibloxfruits(Noitification)",
